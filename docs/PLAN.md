...
| **High** | **Segregation visibility — keep link + summary badge** | Auto-flow (Step 39 #1) silently skips past the segregation review screen once extraction starts. Today the "Review Segregation" link only appears on jobs with `status=analyzed` (a 1-2min window). After that the auditor has no way to see what got classified — including silent LLM fallbacks (`classification_method="fallback"`) and regex fallbacks. Fix: (a) keep link visible on all post-analyze statuses, rename to "Segregation Details" when read-only; (b) summary badge on each job card row like `44 PII · 0 non-PII · 0 failed` with a red variant if any doc has `error != null` or `classification_method="fallback"`. 2-file TSX change. | Small |
...
| Medium | Segregation tab — project-wide flat table | Dedicated tab showing every doc × every job with classification, confidence, method (vision / text / text_late_onset / vision_late_onset / regex_fallback), and any error. Filter + search. Lets auditor audit the whole project's segregation history without clicking into each job. | Medium |
