...
- **LLM model selection (April 2026):** Tested 6 models across 12 files via `scripts/test_llm_judgment.py`. qwen2.5:7b selected as primary text model: 9 critical drifts (vs 14 for 90B), 25s avg (vs 226s for 90B), produces field maps consistently. Config: `OLLAMA_MODEL=qwen2.5:7b` (text), `OLLAMA_VISION_MODEL=llama3.2-vision:90b` (vision-only for scanned/image docs). Segregation routes text PDFs to text model (8.8s vs 150s vision).
...
**Key metrics (April 2026):** 78,471 PII records, 34 docs (PDF/XLSX/XLS/MSG/HEIC/JPG), 30-45ms/page coordinate speed, dual-model (qwen2.5:7b text + llama3.2-vision:90b vision). Phone validity 99.5% (was 85.9%). Gov ID coverage 95% (was 85.2%). Analysis time 3.75min (was 19.7min).
