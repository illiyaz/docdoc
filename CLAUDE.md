...
- **Late-onset PII (Step 30h):** Adaptive onset sampling at 25%/50%/75% of doc catches late-onset PII (e.g., K-1 schedules at page 52 in 100-page tax return). **Known gap:** discovered page feeds into segregation classification but NOT into `content_onset_page` — extraction still starts from page 1. Fix needed: plumb late-onset page → onset to skip boilerplate.
