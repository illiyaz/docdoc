...
| 14. LLM Document Understanding & Detection Quality | PENDING | Three-phase fix for false positive reduction. Phase 14a: deterministic improvements (context deny-lists, tighter Presidio patterns for STUDENT_ID/COMPANY_NUMBER_UK/DRIVER_LICENSE_US/VAT_EU/DATE_OF_BIRTH). Phase 14a-ii: protocol-driven recognizer filtering — pass target_entity_types from protocol config to Presidio so only jurisdiction-relevant recognizers run (GDPR disables US_SSN/US_DRIVER_LICENSE, DPDPA disables UK/EU types, etc.). Phase 14b: LLM Document Understanding — LLM reads onset page, produces DocumentSchema (field map, people, dates, tables, suppression hints), SchemaFilter post-processes Presidio output. Phase 14c: integration with entity analysis + suppression audit trail + frontend. Target: ~85% FP rate → ~10-15% with LLM, ~40-50% without. |
