...
- **LLM-first segregation (Step 30e):** File classification (PII vs non-PII) uses vision LLM on page 1-2, NOT OCR+regex. One LLM call returns: PII yes/no, document type, field inventory, role attribution (primary subject vs secondary contacts), **country_hint (ISO 3166-1 alpha-2, used by gov-ID classifier downstream)**. ~2-3s/file. Two modes: folder (bulk segregation → grouping → auditor review) and single-file (inline segregation → direct analysis). **Late-onset PII detection (1fc16b9):** for docs ≥50 pages where pages 1-2 show no PII, `_check_late_onset_pii` samples 25/50/75% pages and sends each page's text directly to the LLM (no regex gate). When PyMuPDF yields <30 chars on a sampled page — common for form-PDFs where field values sit in AcroForm — the page is rendered and sent to the vision LLM as fallback. Classification method recorded as `text_late_onset` or `vision_late_onset`. Proven on AWIR-482 benchmark: was `pii=false conf=0.95` for 2 runs, flipped to `pii=true` after 1fc16b9. **Known visibility gap (2026-04-19):** auto-flow (Step 39 #1) skips straight past the Segregation Review screen. The "Review Segregation" link is only shown when `job.status=="analyzed"` — a narrow 1-2min window before extraction starts. Post-extraction the auditor loses the visual path back even though the data persists. Silent fallbacks (`classification_method="fallback"` from invalid JSON, or `"regex_fallback"` when Ollama is down) also look identical to LLM-classified docs on the job card. Fix pending as Task #23 (PLAN.md High).
- **Geo-neutral government-ID classifier (`app/pii/gov_id_classifier.py`, f52a448 + e302a01):** Maps raw ID values to canonical types (UK_NINO, IN_PAN, BR_CPF, IT_CF, SG_NRIC, CN_RESID, etc.) across 40+ formats in ~35 countries. Strict alphanumeric formats self-identify; digit-only formats use segregation's `country_hint` (plumbed through `PIIRecord.country` at extraction time) to disambiguate (US_SSN vs NL_BSN vs IL_ID are all 9 digits). Wired at `deduplicator._build_subject` — runs on `raw_government_id` before falling back to entity_types_found. When classifier can't narrow the type AND `country != "US"`, returns `GOVERNMENT_ID` rather than the extractor's hardcoded `"US_SSN"` placeholder. Validated on CMG (UK) + AWIR (US): 100% correct labels across 48 subjects.
- **Completeness-driven vision recovery — targeted (5166546):** When extraction recall is <50%, build a name roster via full-doc batched LLM scan (roster v2, 25019fb — ~5-9 calls per 100pg doc, not 1 call on stratified samples). For each roster name without a gov_id, PyMuPDF text-grep locates its pages. Vision LLM (300 DPI, 90B) targets those pages with a geo-neutral "find the gov ID" prompt. Config: `COMPLETENESS_VISION_STEP/WINDOW/MAX_PAGES` env vars. Fallback: sequential scan if text-grep yields nothing (form-PDFs).
- **Strategy A marker filter — wide-window (0136e67):** Marker-filter extraction captures a window around the name marker; prior default (3 before, 5 after) missed DOB/NI/address rows that sit elsewhere on label-dense pages like pension statements. Filter now accepts `additional_labels` (from `segregation.fields[].name`) and widens its keep-set around each label occurrence too. Strategy A's gov-ID prompt branch recognizes `OTHER_ID` / `NATIONAL_ID` / `TAX_ID` so non-US IDs get requested, not just US_SSN. Proven on CMG: snippet size 95 → 224 chars, DOB coverage 0% → 100%, NI extraction no longer requires vision fallback.
...
**Key metrics (April 2026):** 78,471 PII records, 34 docs (PDF/XLSX/XLS/MSG/HEIC/JPG), 30-45ms/page coordinate speed, dual-model (qwen2.5:32b text + llama3.2-vision:90b vision). Phone validity 99.5% (was 85.9%). Gov ID coverage 95% (was 85.2%). Analysis time 3.75min (was 19.7min). Extraction accuracy 97.3% (32b on school reports). Scale: tested to 500 pages, designed for 3000+. Overnight 12-doc run (April 15): 2,105 records, 1,157 subjects. Adaptive intelligence (April 15-17): Complex1 SSNs 0→50 (field-aware prompts), AWIR-993 SSNs 0→3 (300 DPI vision recovery from K-1 forms), record validation purges ~45% garbage, gap detector 55% fewer false positives. **Benchmark recovery (April 18):** AWIR-482 + CMG joint benchmark went 5 subjects (run 1) → 5 (run 2) → **48 subjects** (run 3: 19 AWIR + 29 CMG, 100% gov-ID-label correctness across US_SSN + UK_NINO, 100% address, 90% DOB). Fix set: country_hint plumbing, regex-gate drop + vision fallback in late-onset segregation, roster v2 + text-grep targeted recovery, Strategy A marker filter widened to capture labeled DOB/NI windows, names_match normalizes inputs internally.
