...
if [ -f "$WORKSTATE" ]; then
    WORKSTATE_LINES_BEFORE=$(wc -l < "$WORKSTATE")
fi

# Snapshot git state
GIT_DIFF_BEFORE=$(git diff --stat 2>/dev/null | tail -1 || echo "no git")
FILES_BEFORE=$(git diff --name-only 2>/dev/null | wc -l || echo "0")
...
# Count checkpoints written (lines in Checkpoint Log section)
CHECKPOINT_COUNT=0
if [ -f "$WORKSTATE" ]; then
...
if [ -f "$WORKSTATE" ]; then
    WORKSTATE_LINES_AFTER=$(wc -l < "$WORKSTATE")
...
FILES_MODIFIED=$(grep -c "✓\|DONE" "$WORKSTATE" 2>/dev/null || echo "0")
FILES_CREATED=$(grep -c "created\|Created\|NEW" "$WORKSTATE" 2>/dev/null || echo "0")
...
TASK_COMPLETE="false"
if grep -qi "complete\|done\|finished" "$WORKSTATE" 2>/dev/null | head -1 | grep -qi "Task Status"; then
    TASK_COMPLETE="true"
fi
...
# (heuristic: agents don't checkpoint this often unless recovering from compaction)
...
{"run_id": "$RUN_ID", "task": "$(echo "$TASK" | head -c 100 | sed 's/"/\\"/g')", "start": "$START_TIME", "end": "$END_TIME", "duration_seconds": $DURATION, "checkpoints_written": $CHECKPOINT_COUNT, "estimated_compactions": $ESTIMATED_COMPACTIONS, "files_modified": $FILES_MODIFIED, "files_created": $FILES_CREATED, "remaining_items": $REMAINING, "task_complete": $TASK_COMPLETE, "workstate_changed": $WORKSTATE_CHANGED, "workstate_lines_before": $WORKSTATE_LINES_BEFORE, "workstate_lines_after": $WORKSTATE_LINES_AFTER, "git_files_changed": $FILES_AFTER, "git_lines_added": $LINES_ADDED, "git_lines_removed": $LINES_REMOVED}
METRICS_JSON
